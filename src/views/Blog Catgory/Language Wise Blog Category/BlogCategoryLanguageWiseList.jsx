...
import { ChangeBlogCategoryStatus, GetBlogCategoryList } from 'services/Blog/BlogCategoryApi'
import BlogCategoryLanguageWiseAddUpdateModal from './BlogCategoryLanguageWiseAddUpdateModal'

const BlogCategoryLanguageWiseList = () => {
      ...
      useEffect(() => {
            GetBlogCategoryListData(1, null)
      }, [])
      const location = useLocation()
      useEffect(() => {
            if (isAddUpdateDone) {
                  GetBlogCategoryListData(currentPage, null)
      ...
      const GetBlogCategoryListData = async (pageNumber, searchKeywordValue) => {
            setLoader(true);
            try {
                  ...
                  }, location.state.blogCategoryData.blogCategoryKeyID);
      ...
      console.log(location?.state?.blogCategoryData, 'dsad324r32');

      const AddShopBtnClicked = () => {
            setModelRequestData((prev) => ({
                  ...prev, Action: null,
                  availableLangID: availableLangIDs, shopID: null, blogCategoryKeyID: location?.state?.blogCategoryData?.blogCategoryKeyID
            }))
      ...
      const updateBtnClickPujaCat = (value) => {
            setModelRequestData((prev) => ({
                  ...prev, Action: 'Update',
                  blogCategoryKeyID: value?.blogCategoryKeyID,
      ...
      const confirmStatusChange = async (item, user) => {
            try {
                  const { blogCategoryKeyID, appLangID } = item; // Destructure to access only what's needed
                  const response = await ChangeBlogCategoryStatus(blogCategoryKeyID, appLangID);
      ...
      const fetchSearchResults = (searchValue) => {
            GetBlogCategoryListData(1, searchValue);
      };
      ...
      const handlePageChange = (pageNumber) => {
            ...
            GetBlogCategoryListData(pageNumber, null);
      };
      ...
      return (
            <>
                                                      ...
                                                      {ShopList?.map((item, idx) => (
                                                            <tr className='text-nowrap text-center' key={item.idx}>
                                                                  ...
                                                                  <td className="text-center">
                                                                        {item.blogCatName?.length > 30 ? (
                                                                              <Tooltip title={item.blogCatName}>{`${item.blogCatName?.substring(0, 30)}...`}</Tooltip>
                                                                        ) : (
                                                                              <>{item.blogCatName}</>
                                                                        )}
                                                                  </td>
                  ...
                  {showAddUpdateModal &&
                        ...
                        <BlogCategoryLanguageWiseAddUpdateModal show={showAddUpdateModal} onHide={(() => setShowAddUpdateModal(false))} modelRequestData={modelRequestData} setIsAddUpdateDone={setIsAddUpdateDone} />
                  }
                  <ImagePreviewModal
                        show={showModal}
...
export default BlogCategoryLanguageWiseList
