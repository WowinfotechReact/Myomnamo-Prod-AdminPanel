...
const LanguageWiseBlogList = () => {
      ...
      const GetWareHouseListData = async (pageNumber, searchKeywordValue) => {
            ...
            try {
                  ...
                  }, location.state.blogCategoryData.blogKeyID);
      ...
      console.log(availableLangIDs, 's32432');

      const AddShopBtnClicked = () => {
            setModelRequestData((prev) => ({
                  ...prev, Action: null, shopID: null,
                  // blogKeyID: location.
      ...
      const confirmStatusChange = async (item, user) => {
            try {
                  const { blogKeyID, appLangID } = item; // Destructure to access only what's needed
                  const response = await ChangeBlogStatus(blogKeyID, appLangID);
      ...
      return (
            <>
                  ...
                  {showAddUpdateModal &&
                        ...
                        <LanguageWiseAddUpdateBlogModal show={showAddUpdateModal} onHide={(() => setShowAddUpdateModal(false))} modelRequestData={modelRequestData} setIsAddUpdateDone={setIsAddUpdateDone} />
                  }
                  <ImagePreviewModal
                        show={showModal}
...
export default LanguageWiseBlogList
