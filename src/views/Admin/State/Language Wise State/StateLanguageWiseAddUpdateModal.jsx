...
const StateLanguageWiseAddUpdateModal = ({ show, onHide, modelRequestData, setIsAddUpdateDone }) => {
      ...
      const [blogCatObj, setBlogCatObj] = useState({
            ...
            stateName: null,
            stateByLangKeyID: null,
            appLangID: null,
      })
      // console.log("module object ==>>", modelRequestData)
      useEffect(() => {
      ...
      const SubmitBtnClicked = () => {
            let isValid = true
            if (
                  blogCatObj?.stateName === null || blogCatObj?.stateName === undefined || blogCatObj?.stateName === "" ||
      ...
      const GetStateModelData = async (id) => {
            ...
            setLoader(true);
            try {
                  const response = await GetStateCategoryModel(id, modelRequestData?.appLangID);
                  if (response) {
                        if (response?.data?.statusCode === 200) {
                              setLoader(false);
                              if (response?.data?.responseData?.data) {
                                    ...
                                    setBlogCatObj((prev) => ({
                                          ...
                                          stateByLangKeyID: List.stateByLangKeyID
      ...
      return (
            <>
                  <Modal style={{ zIndex: 1300 }} size='md' show={show} backdrop="static" keyboard={false} centered>
                        <Modal.Header>
                              <h4 className="text-center">{modelRequestData?.Action === null ? 'Add State Language' : 'Update State Language'}</h4>
                        </Modal.Header>
                              ...
                              <div className="container-fluid ">
                                    <div className="row">
                                          ...
                                          <div className="col-md-12 mb-3">
                                                ...
                                                />
                                                {error && (blogCatObj.stateName === null || blogCatObj.stateName === undefined || blogCatObj.stateName === '') ? (
...
export default StateLanguageWiseAddUpdateModal
