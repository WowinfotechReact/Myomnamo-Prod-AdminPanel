...
import StateLanguageWiseAddUpdateModal from './StateLanguageWiseAddUpdateModal'
import { ChangeStateStatus, GetStateList } from 'services/State/StateApi'

const StateLanguageWiseList = () => {
      ...
      const GetStateListData = async (pageNumber, searchKeywordValue) => {
            setLoader(true);
            try {
                  ...
                  }, location.state.stateData.stateKeyID);
      ...
      const updateBtnClickPujaCat = (value) => {
            setModelRequestData((prev) => ({
                  ...prev, Action: 'Update',
                  stateKeyID: value?.stateKeyID,
                  appLangID: value.appLangID,
                  availableLangID: availableLangIDs,
                  stateByLangKeyID: value?.stateByLangKeyID,
                  status: value?.status
            }))
            setShowAddUpdateModal(true)
      }
      ...
      const confirmStatusChange = async (item, user) => {
            try {
                  const { stateKeyID, appLangID } = item; // Destructure to access only what's needed
                  const response = await ChangeStateStatus(stateKeyID, appLangID);
      ...
      const fetchSearchResults = (searchValue) => {
            GetStateListData(1, searchValue);
      };
      ...
      const handlePageChange = (pageNumber) => {
            ...
            GetStateListData(pageNumber, null);
      };
      ...
      return (
            <>
                  ...
                  {showAddUpdateModal &&
                        <StateLanguageWiseAddUpdateModal show={showAddUpdateModal} onHide={(() => setShowAddUpdateModal(false))} modelRequestData={modelRequestData} setIsAddUpdateDone={setIsAddUpdateDone} />
                  }
...
export default StateLanguageWiseList
