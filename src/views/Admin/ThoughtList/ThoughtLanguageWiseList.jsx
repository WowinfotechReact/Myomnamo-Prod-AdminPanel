...
const ThoughtLanguageWiseList = () => {
  ...
  const GetThoughtListData = async () => {
    setLoader(true);
    try {
      const response = await GetThoughtList(
        ...
        location?.state?.data.thoughtKeyID
      );
  ...
  const fetchSearchResults = (searchValue) => {
    GetThoughtListData(1, searchValue);
  };
  ...
  const handlePageChange = (pageNumber) => {
    ...
    GetThoughtListData(pageNumber, null);
  };
  ...
  const AddBtnClicked = () => {
    setModelRequestData((prev) => ({
      ...
      Action: null,
      appLangID: null,
      thoughtKeyID: location?.state?.data?.thoughtKeyID,
      thoughtByLangKeyID: null,
  ...
  const updateBtnClickPujaCat = (value) => {
    setModelRequestData((prev) => ({
      ...
      Action: 'update',
      thoughtKeyID: location?.state?.data?.thoughtKeyID,
      thoughtByLangKeyID: value?.thoughtByLangKeyID,
                ...
                <tbody>
                  {thoughtList?.map((item, idx) => (
                    <tr className="text-nowrap text-center" key={item.idx}>
                      ...
                      <td style={{ whiteSpace: 'nowrap' }}>{item.thoughtName === null ? '-' : item.thoughtName}</td>
...
export default ThoughtLanguageWiseList;
