...
import { AddUpdateThoughtAPI, GetThoughtModalAPI } from 'services/Admin/ThoughtMasterAPI/ThoughtMasterAPI';

const AddUpdateThoughtModal = ({ show, onHide, modelRequestData, setIsAddUpdateDone }) => {
  ...
  useEffect(() => {
    if (show) {
      ...
      if (modelRequestData?.Action === 'update' && modelRequestData?.thoughtKeyID !== null && modelRequestData?.thoughtKeyID !== '') {
        GetModelData();
      }
    }
  }, [show]);
  ...
  const GetModelData = async () => {
    ...
    setLoader(true);
    try {
      const response = await GetThoughtModalAPI(modelRequestData?.thoughtKeyID, modelRequestData?.appLangID);
  ...
  const SubmitBtnClicked = () => {
    let isValid = true;
    if (formObj.thought === null || formObj.thought === undefined || formObj.thought === '') {
      setError(true);
      isValid = false;
    }
    ...
    if (modelRequestData.moduleName === 'LanguageWiseList') {
      if (
        formObj.thought === null ||
        formObj.thought === undefined ||
        formObj.thought === '' ||
        formObj.appLangID === null ||
        formObj.appLangID === undefined ||
        formObj.appLangID === ''
      ) {
        setError(true);
        isValid = false;
    ...
    const apiParam = {
      ...
      thoughtName: formObj.thought
    };

    if (isValid) {
      AddUpdateThoughtData(apiParam);
    }
  };

  const AddUpdateThoughtData = async (ApiParam) => {
    setLoader(true);
    try {
          ...
          <div className="container-fluid ">
            <div className="row">
              ...
              <div className="col-md-12 mb-3">
                ...
                />
                {error && (formObj.thought === null || formObj.thought === undefined || formObj.thought === '') ? (
...
export default AddUpdateThoughtModal;
