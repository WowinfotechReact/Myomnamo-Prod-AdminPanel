...
const [showErrorModal, setShowErrorModal] = useState(false);
  ...
  // const allSelected = thoughtList?.length > 0 && selectedBookings.length === thoughtList.length;
  ...
  useEffect(() => {
    if (isAddUpdateDone) {
      ...
      setSelectedBookings([]);
      GetThoughtMasterListData(currentPage, null);
      setIsAddUpdateDone(false);
    }
  }, [isAddUpdateDone]);

  // ------------API Callings--------------------
  const GetThoughtMasterListData = async (pageNo, searchValue) => {
    setLoader(true);
    try {
      const response = await GetThoughtList({
        // pageSize: pageSize,
        // pageNo: pageNumber - 1,
  ...
  const updateBtnClicked = (value) => {
    setModelRequestData((prev) => ({
      ...prev,
      Action: 'update',
      thoughtKeyID: value?.thoughtKeyID,
      appLangID: value.appLangID
    }));
    setShowAddModal(true);
  };
  const fetchSearchResults = (searchValue) => {
    GetThoughtMasterListData(currentPage, searchValue);
  };
  ...
  //     setSelectedBookings([]);
  //   } else {
  //     setSelectedBookings(thoughtList.map((item) => item.bookingID));
  //   }
  // };
  ...
  const AddLangBtnClicked = (value) => {
    navigate('/thought-language-wise', {
      state: {
        data: value
      }
    });
                  ...
                  {thoughtList?.map((item, idx) => (
                    <tr className="text-nowrap text-center" key={item.pujaBookingID}>
                      ...
                      <td style={{ whiteSpace: 'nowrap' }}>
                        {item?.thoughtName ? (
                          item.thoughtName.length > 25 ? (
                            <Tooltip title={item.thoughtName}>{item.thoughtName.substring(0, 25) + '...'}</Tooltip>
                          ) : (
                            item.thoughtName
                          )
                        ) : (
...
export default ThoughtMasterList;
