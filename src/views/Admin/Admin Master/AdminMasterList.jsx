...
import { GetAdminList } from 'services/Admin/Admin Master/AdminMasterApi'
import AdminMasterAddUpdateModal from './AdminMasterAddUpdateModal'
...
const AdminMasterList = () => {
    ...
    const [showCurrentPassword, setCuurrentPassword] = useState(false);
    ...
    const confirmStatusChange = async (item, user) => {
        try {
            const { AdminKeyID } = item; // Destructure to access only what's needed
            const response = await ChangeDistrictStatus(AdminKeyID);
            ...
            if (response && response.data.statusCode === 200) {
                ...
                setModelAction('Admin status changed successfully.');
            } else {
                console.error(response?.data?.errorMessage);
                setShowSuccessModal(true);
                setModelAction('Failed to change admin status.');
            }
        } catch (error) {
    ...
    const HandleViewqPassword = (item) => {
        setShowPassword(!showPassword)
        setCuurrentPassword(item?.adminName)
    }
    return (
        <>
            <div className="card">
                <div className="card-body p-2 bg-white shadow-md rounded-lg" style={{ borderRadius: '10px' }}>
                    <div className="d-flex justify-content-between align-items-center mb-1">
                        <h5 className="m-0">Admin Master</h5>
                                    ...
                                    {ShopList?.map((item, idx) => (
                                        <tr className='text-nowrap text-center' key={item.idx}>
                                            ...
                                            <td className="text-center py-2">
                                                {item.adminName?.length > 30 ? (
                                                    <Tooltip title={item.adminName}>{`${item.adminName?.substring(0, 30)}...`}</Tooltip>
                                                ) : (
                                                    <>{item.adminName}</>
                                                )}
                                            </td>
                                            ...
                                            <td className="text-center py-2">
                                                <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "6px" }}>
                                                    {showPassword && (showCurrentPassword === item?.adminName) ? (
                                                    ...
                                                    <span
                                                        style={{ cursor: "pointer" }}
                                                        onClick={() => HandleViewqPassword(item)}
                                                    >
                                                        {showPassword && (showCurrentPassword === item?.adminName) ? <FaEyeSlash /> : <FaEye />}
                                                    </span>
                                                </div>
                                            </td>
                                            <td className="text-center py-2">
                                                {item.roleType?.length > 30 ? (
                                                    <Tooltip title={item.roleType}>{`${item.roleType?.substring(0, 30)}...`}</Tooltip>
                                                ) : (
                                                    <>{item.roleType}</>
                                                )}
                                            </td>
            ...
            {showAddUpdateModal &&
                <AdminMasterAddUpdateModal show={showAddUpdateModal} onHide={(() => setShowAddUpdateModal(false))} modelRequestData={modelRequestData} setIsAddUpdateDone={setIsAddUpdateDone} />
            }
            <ImagePreviewModal
                show={showModal}
