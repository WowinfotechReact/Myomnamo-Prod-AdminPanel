...
import { GetImageCatList } from 'services/Admin/ImageMasterAPI/ImageCategoryAPI';
import AddUpdateImageCatModal from './AddUpdateImageCategory';
...
const ImageCatList = () => {
  ...
  const [imageCatList, setImageCatList] = useState([]);
  const [toDate, setToDate] = useState(null);
  const [fromDate, setFromDate] = useState(null);
  const [selectedBookings, setSelectedBookings] = useState([]);

  // ✅ Check if all rows are selected
  const selectableRows = imageCatList.filter((item) => item.panditName === null);
  // const allSelected = imageCatList?.length > 0 && selectedBookings.length === imageCatList.length;
  ...
  useEffect(() => {
    if (location?.pathname === '/image-category') {
      setPageHeading('Image Category');
      GetImageCategoryListData(1, null, pujaServiceID?.PanditPuja, pujaSubServiceID?.PanditPuja);
    } else {
  ...
  useEffect(() => {
    if (isAddUpdateDone) {
      ...
      setSelectedBookings([]);
      GetImageCategoryListData(currentPage, null);
      setIsAddUpdateDone(false);
    }
  }, [isAddUpdateDone]);

  // ------------API Callings--------------------
  const GetImageCategoryListData = async (pageNo, searchValue) => {
    setLoader(true);
    try {
      const response = await GetImageCatList({
        pageSize: pageSize,
        pageNo: pageNo - 1,
  ...
  const fetchSearchResults = (searchValue) => {
    GetImageCategoryListData(currentPage, searchValue);
  };
  ...
  //     setSelectedBookings([]);
  //   } else {
  //     setSelectedBookings(imageCatList.map((item) => item.bookingID));
  //   }
  // };
            ...
            <div className="d-flex gap-2 align-items-center">
              <Tooltip title="Add Image Cat">
                <button onClick={handleAddModal} className="btn btn-primary btn-sm" style={{ cursor: 'pointer' }}>
                  ...
                  <span className="d-none d-sm-inline">Add Image Cat</span>
                </button>
              </Tooltip>
            </div>
          </div>
