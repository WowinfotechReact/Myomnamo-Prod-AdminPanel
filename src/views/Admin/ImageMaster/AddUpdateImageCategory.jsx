...
import { AddUpdateImageCatAPI, GetImageCatModalAPI } from 'services/Admin/ImageMasterAPI/ImageCategoryAPI';

const AddUpdateImageCatModal = ({ show, onHide, modelRequestData, setIsAddUpdateDone }) => {
  ...
  const GetModelData = async () => {
    setLoader(true);
    try {
      const response = await GetImageCatModalAPI(modelRequestData?.productCatKeyID);
  ...
  const SubmitBtnClicked = () => {
    ...
    if (isValid) {
      AddUpdateImageCatData(apiParam);
    }
  };

  const AddUpdateImageCatData = async (ApiParam) => {
    setLoader(true);
    try {
      const response = await AddUpdateImageCatAPI(ApiParam);
      if (response?.data?.statusCode === 200) {
        setLoader(false);
        setActionMassage(modelRequestData.Action === null ? `Image Category added successfully.` : `Image Category updated successfully.`);
  ...
  return (
    <>
      <Modal style={{ zIndex: 1300 }} size="md" show={show} backdrop="static" keyboard={false} centered>
        <Modal.Header>
          <h4 className="text-center">{modelRequestData?.Action === null ? `Add Image Category` : `Update Image Category`}</h4>
        </Modal.Header>
            ...
            <div className="row">
              ...
              <div className="col-md-12 mb-3">
                <label htmlFor="pujaName" className="form-label">
                  Image Category Name <span className="text-danger">*</span>
                </label>
                <input
                  maxLength={90}
                  type="text"
...
export default AddUpdateImageCatModal;
