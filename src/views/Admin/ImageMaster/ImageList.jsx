...
import AddUpdateImageModal from './AddUpdateImage';
...
const ImageList = () => {
  ...
  useEffect(() => {
    if (location?.pathname === '/image') {
      setPageHeading('Image');
      GetImageListData(1, null, pujaServiceID?.PanditPuja, pujaSubServiceID?.PanditPuja);
  ...
  const GetImageListData = async (pageNo, searchValue) => {
    setLoader(true);
    try {
      const response = await GetImageListAPI({
        pageSize: pageSize,
        pageNo: pageNo - 1,
  ...
  const ChangeStatusData = async (value) => {
    ...
    setLoader(true);
    try {
      const response = await ChangeImageStatus(value?.imageKeyID);
