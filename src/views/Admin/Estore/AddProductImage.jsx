...
import { AddUpdateProductImageAPI, GetProductImageModalAPI } from 'services/Admin/EStoreAPI/ProductAPI';

const AddUpdateProductImage = ({ show, onHide, modelRequestData, setIsAddUpdateDone }) => {
  ...
  useEffect(() => {
    if (show) {
      ...
      if (modelRequestData?.Action === 'update' && modelRequestData?.piid !== null && modelRequestData?.piid !== '') {
        GetModelData();
      }
    }
  }, [show]);
  ...
  const GetModelData = async () => {
    setLoader(true);
    try {
      const response = await GetProductImageModalAPI(modelRequestData?.piid);
      ...
      if (response) {
        if (response?.data?.statusCode === 200) {
          ...
          if (response?.data?.responseData?.data) {
            ...
            setFilePreview(List.productImageURL);
            setUploadedImageUrl(List.productImageURL);
          }
        } else {
  ...
  const SubmitBtnClicked = () => {
    let isValid = true;
    if (uploadedImageUrl === null || uploadedImageUrl === undefined || uploadedImageUrl === '') {
      setError(true);
      isValid = false;
    }
    ...
    const apiParam = {
      ...
      piid: modelRequestData?.piid ? modelRequestData?.piid : null,
  ...
  const AddUpdateImageData = async (ApiParam) => {
    setLoader(true);
    try {
      const response = await AddUpdateProductImageAPI(ApiParam);
      if (response?.data?.statusCode === 200) {
        setLoader(false);
