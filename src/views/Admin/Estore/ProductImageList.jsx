...
import { ChangeImageStatus, GetImageListAPI } from 'services/Admin/ImageMasterAPI/ImageMasterAPI';
import ImagePreviewModal from 'Image Preview Modal/ImagePreviewModal';
import AddUpdateProductImage from './AddProductImage';
import { ChangeProductImageStatus, GetProductImageListAPI } from 'services/Admin/EStoreAPI/ProductAPI';
...
const ProductImageList = () => {
  ...
  const selectableRows = imageList.filter((item) => item.panditName === null);
  // const allSelected = imageList?.length > 0 && selectedBookings.length === imageList.length;
  ...
  useEffect(() => {
    if (location?.pathname === '/add-image') {
      setPageHeading(location?.state?.moduleName === 'Prasad' ? 'Prasad Image' : 'Product Image');
      GetImageListData(1, null);
    } else {
  ...
  useEffect(() => {
    if (isAddUpdateDone) {
      ...
      setSelectedBookings([]);
      GetImageListData(currentPage, null, modelRequestData?.pujaServiceID, modelRequestData?.pujaSubServiceID);
      setIsAddUpdateDone(false);
    }
  }, [isAddUpdateDone]);

  // ------------API Callings--------------------
  const GetImageListData = async (pageNo, searchValue) => {
    setLoader(true);
    try {
      const response = await GetProductImageListAPI(
        {
          pageSize: pageSize,
          pageNo: pageNo - 1,
  ...
  const ChangeStatusData = async (value) => {
    ...
    setLoader(true);
    try {
      const response = await ChangeProductImageStatus(value?.piid);
  ...
  const fetchSearchResults = (searchValue) => {
    GetImageListData(currentPage, searchValue);
  };
  ...
  //     setSelectedBookings([]);
  //   } else {
  //     setSelectedBookings(imageList.map((item) => item.bookingID));
  //   }
  // };
              ...
              <Tooltip title="Add Image">
                <button onClick={handleAddModal} className="btn btn-primary btn-sm" style={{ cursor: 'pointer' }}>
                  ...
                  <span className="d-none d-sm-inline">
                    {location.state.moduleName === 'Prasad' ? 'Add Prasad Image' : 'Add Product Image'}
                  </span>
                </button>
              </Tooltip>
            </div>
                  ...
                  {imageList?.map((item, idx) => (
                    <tr className="text-nowrap text-center" key={item.pujaBookingID}>
                      ...
                      {/* <td style={{ whiteSpace: 'nowrap' }}>
                        {item?.imageCatName ? (
                          item.imageCatName.length > 25 ? (
                            <Tooltip title={item.imageCatName}>{item.imageCatName.substring(0, 25) + '...'}</Tooltip>
                          ) : (
                            item.imageCatName
                          )
                        ) : (
                      ...
                      <td style={{ whiteSpace: 'nowrap', cursor: 'pointer' }}>
                        {item.productImageURL ? (
                          <img
                            ...
                            alt={item.blogTitle}
                            style={{ width: '60px', height: '60px', objectFit: 'contain', borderRadius: '5px' }}
                          />
                        ) : (
                          'No Image'
                        )}
                      ...
                      <td className="text-center actionColSticky" style={{ zIndex: 4 }}>
                        <div className="d-flex justify-content-center gap-2 mt-2">
                          ...
                          <Tooltip title="Copy Image URL">
                            <Button style={{ marginRight: '5px' }} className="btn-sm" onClick={() => handleCopyPath(item.productImageURL)}>
