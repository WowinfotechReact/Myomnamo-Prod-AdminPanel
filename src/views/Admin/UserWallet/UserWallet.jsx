...
import AddUpdateUserWalletModal from './AddUpdateUserWallet';
import { GetUserWalletList } from 'services/Admin/UserWalletAPI/UserWalletAPI';
...
const UserWalletList = () => {
  ...
  const selectableRows = userWalletList.filter((item) => item.panditName === null);
  // const allSelected = userWalletList?.length > 0 && selectedBookings.length === userWalletList.length;
  ...
  useEffect(() => {
    if (location?.pathname === '/user-wallet') {
      setPageHeading('User Wallet');
      GetUserWalletBookingListData(1, null, pujaServiceID?.PanditPuja, pujaSubServiceID?.PanditPuja);
  ...
  useEffect(() => {
    if (isAddUpdateDone) {
      ...
      setSelectedBookings([]);
      GetUserWalletBookingListData(currentPage, null);
      setIsAddUpdateDone(false);
    }
  }, [isAddUpdateDone]);

  // ------------API Callings--------------------
  const GetUserWalletBookingListData = async (pageNo, searchValue) => {
    setLoader(true);
    try {
  ...
  const updateBtnClicked = (value) => {
    setModelRequestData((prev) => ({
      ...
      referredByUserID: [value?.referredByUserID],
      walletAmount: value?.walletAmount
    }));
    setShowAddModal(true);
  };
  const fetchSearchResults = (searchValue) => {
    GetUserWalletBookingListData(currentPage, searchValue);
  };
  ...
  const handlePageChange = (pageNumber) => {
    ...
    setUserWalletList([]);
    GetUserWalletBookingListData(pageNumber, null);
  };
  ...
  //     setSelectedBookings([]);
  //   } else {
  //     setSelectedBookings(userWalletList.map((item) => item.bookingID));
  //   }
  // };
            ...
            <div className="d-flex gap-2 align-items-center">
              <Tooltip title="Add Wallet">
                <button onClick={handleAddModal} className="btn btn-primary btn-sm" style={{ cursor: 'pointer' }}>
                  ...
                  <span className="d-none d-sm-inline">Add Wallet</span>
                </button>
              </Tooltip>
            </div>
          </div>
