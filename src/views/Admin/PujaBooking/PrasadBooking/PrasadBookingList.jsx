...
import PrasadBookingOrderDetails from './PrasadBookingOrderDetails';
import { GetPrasadBookingList } from 'services/Admin/PujaBookingAPI/PrasadBookingAPI';
// import EstoreBookingOrderDetails from './EstoreOrderDetailsModal';
// import AddUpdatePujaModal from './AddUpdatePujaModal';

const PrasadBookingList = () => {
  ...
  const [prasadBookingList, setPrasadBookingList] = useState([]);
  ...
  const selectableRows = prasadBookingList.filter((item) => item.panditName === null);
  // const allSelected = prasadBookingList?.length > 0 && selectedBookings.length === prasadBookingList.length;
  ...
  useEffect(() => {
    if (location?.pathname === '/prasad-bookings') {
      setPageHeading('Prasad Booking List');
      GetPrasadBookingListData(1, null);
  ...
  useEffect(() => {
    if (isAddUpdateDone) {
      ...
      setSelectedBookings([]);
      GetPrasadBookingListData(currentPage, null, modelRequestData?.pujaServiceID, modelRequestData?.pujaSubServiceID);
      setIsAddUpdateDone(false);
    }
  }, [isAddUpdateDone]);

  // ------------API Callings--------------------
  const GetPrasadBookingListData = async (pageNumber, searchKeywordValue, pujaServiceID, pujaSubServiceID) => {
    setLoader(true);
    try {
      const response = await GetPrasadBookingList({
        pageSize: pageSize,
        pageNo: pageNumber - 1,
  ...
  const handlePageChange = (pageNumber) => {
    ...
    setPrasadBookingList([]);
    GetPrasadBookingListData(pageNumber, null, modelRequestData?.pujaServiceID, modelRequestData?.pujaSubServiceID);
  };
  ...
  //     setSelectedBookings([]);
  //   } else {
  //     setSelectedBookings(prasadBookingList.map((item) => item.bookingID));
  //   }
  // };
  ...
  const handleStatusChange = async (PujaBookingKeyID, newStatus) => {
    ...
    try {
      ...
      await ChangeEstoreOrderStatus(PujaBookingKeyID, newStatus);
      setLoader(false);
      ...
      await GetPrasadBookingListData(currentPage, searchKeyword, modelRequestData?.pujaServiceID, modelRequestData?.pujaSubServiceID);
      setLoader(false);
    }
  };
          ...
          <div>
            <div className="table-responsive" style={{ maxHeight: '65vh' }}>
              <Table striped bordered hover>
                ...
                <tbody>
                  {prasadBookingList?.map((item, idx) => (
...
export default PrasadBookingList;
