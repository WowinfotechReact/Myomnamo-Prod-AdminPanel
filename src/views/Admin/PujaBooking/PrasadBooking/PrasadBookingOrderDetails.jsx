...
import { GetPrasadBookingOrderDetailsAPI } from 'services/Admin/PujaBookingAPI/PrasadBookingAPI';
import { GetPujaBookingOrderDetailsAPI } from 'services/Admin/PujaBookingAPI/PujaBookingAPI';

const PrasadBookingOrderDetails = ({ show, onHide, modelRequestData }) => {
  ...
  useEffect(() => {
    if (show) {
      PrasadBookingOrderDetailsData();
    }
  }, [show]);

  const PrasadBookingOrderDetailsData = async () => {
    // debugger;
    setLoader(true);
    try {
      const response = await GetPrasadBookingOrderDetailsAPI(modelRequestData?.estoreBookingKeyID);
  ...
  return (
    <Modal style={{ zIndex: 1300 }} show={show} backdrop="static" keyboard={false} centered size="lg">
      <Modal.Header>
        <h4 className="text-center">Prasad Booking Order Details </h4>
      </Modal.Header>
      <Modal.Body>
        {orderBookingData && (
          <>
...
export default PrasadBookingOrderDetails;
