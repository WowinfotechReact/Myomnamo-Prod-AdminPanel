...
const NewsLetterList = () => {
  ...
  const selectableRows = newsletterList.filter((item) => item.panditName === null);
  // const allSelected = newsletterList?.length > 0 && selectedBookings.length === newsletterList.length;
  ...
  useEffect(() => {
    if (location?.pathname === '/newsletter') {
      setPageHeading('NewsLetter Subscribers List');
      GetNewsLetterListData(1, null, pujaServiceID?.PanditPuja, pujaSubServiceID?.PanditPuja);
  ...
  useEffect(() => {
    if (isAddUpdateDone) {
      ...
      setSelectedBookings([]);
      GetNewsLetterListData(currentPage, null);
      setIsAddUpdateDone(false);
    }
  }, [isAddUpdateDone]);

  // ------------API Callings--------------------
  const GetNewsLetterListData = async (pageNo, searchValue) => {
    setLoader(true);
    try {
  ...
  const fetchSearchResults = (searchValue) => {
    GetNewsLetterListData(currentPage, searchValue);
  };
                      ...
                      <td style={{ whiteSpace: 'nowrap' }}>
                        {item?.fName ? (
                          item.fName.length > 25 ? (
                            <Tooltip title={item.fName}>{item.fName.substring(0, 25) + '...'}</Tooltip>
                          ) : (
                            item.fName
                          )
                        ) : (
                      ...
                      <td style={{ whiteSpace: 'nowrap' }}>
                        {item?.lName ? (
                          item.lName.length > 25 ? (
                            <Tooltip title={item.lName}>{item.lName.substring(0, 25) + '...'}</Tooltip>
                          ) : (
                            item.lName
                          )
                        ) : (
