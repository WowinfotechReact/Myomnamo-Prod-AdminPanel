...
const FAQList = () => {
  ...
  // const allSelected = faqList?.length > 0 && selectedBookings.length === faqList.length;
  ...
  useEffect(() => {
    if (isAddUpdateDone) {
      ...
      setSelectedBookings([]);
      GetFAQListData(currentPage, null, modelRequestData?.pujaServiceID, modelRequestData?.pujaSubServiceID);
      setIsAddUpdateDone(false);
    }
  }, [isAddUpdateDone]);

  // ------------API Callings--------------------
  const GetFAQListData = async (pageNo, searchValue) => {
    setLoader(true);
    try {
  ...
  const updateBtnClicked = (value) => {
    setModelRequestData((prev) => ({
      ...prev,
      Action: 'update',
      faqKeyID: value?.faqKeyID,
      appLangID: value.appLangID
    }));
    setShowAddModal(true);
  };
  const fetchSearchResults = (searchValue) => {
    GetFAQListData(currentPage, searchValue);
  };
  ...
  //     setSelectedBookings([]);
  //   } else {
  //     setSelectedBookings(faqList.map((item) => item.bookingID));
  //   }
  // };
            ...
            <div className="d-flex gap-2 align-items-center">
              <Tooltip title="Add FAQ">
                <button onClick={handleAddModal} className="btn btn-primary btn-sm" style={{ cursor: 'pointer' }}>
                  ...
                  <span className="d-none d-sm-inline">Add FAQ</span>
                </button>
              </Tooltip>
            </div>
          </div>
                ...
                <tbody>
                  {faqList?.map((item, idx) => (
                    <tr className="text-nowrap text-center" key={item.pujaBookingID}>
                      ...
                      <td style={{ whiteSpace: 'nowrap' }}>
                        {item?.question ? (
                          item.question.length > 25 ? (
                            <Tooltip title={item.question}>{item.question.substring(0, 25) + '...'}</Tooltip>
                          ) : (
                            item.question
                          )
                        ) : (
                      ...
                      <td style={{ whiteSpace: 'nowrap' }}>
                        {item?.answer ? (
                          item.answer.length > 25 ? (
                            <Tooltip title={item.answer}>{item.answer.substring(0, 25) + '...'}</Tooltip>
                          ) : (
                            item.answer
                          )
                        ) : (
