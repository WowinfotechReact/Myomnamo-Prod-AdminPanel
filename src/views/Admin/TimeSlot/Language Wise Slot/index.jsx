...
import TimeSlotLanguageWiseAddUpdateModal from './TimeSlotLanguageWiseAddUpdateModal'
...
const TimeSlotLanguageWiseList = () => {
    ...
    const GetTimeSlotListData = async (pageNumber, searchKeywordValue) => {
        ...
        try {
            ...
            }, location.state.stateData.timeSlotKeyID);
    ...
    const updateBtnClickPujaCat = (value) => {
        setModelRequestData((prev) => ({
            ...
            timeSlotByLangKeyID: value?.timeSlotByLangKeyID,
            status: value?.status
        }))
        setShowAddUpdateModal(true)
    }
    ...
    const confirmStatusChange = async (item, user) => {
        try {
            const { timeSlotKeyID, appLangID } = item; // Destructure to access only what's needed
            const response = await ChangeTimeSlotStatus(timeSlotKeyID, appLangID);
    ...
    return (
        <>
            ...
            {showAddUpdateModal &&
                <TimeSlotLanguageWiseAddUpdateModal show={showAddUpdateModal} onHide={(() => setShowAddUpdateModal(false))} modelRequestData={modelRequestData} setIsAddUpdateDone={setIsAddUpdateDone} />
            }
...
export default TimeSlotLanguageWiseList
