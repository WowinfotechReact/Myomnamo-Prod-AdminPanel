...
import { GetEmployeeMasterList } from 'services/Admin/Employee Master/EmployeeMasterApi'
import EmployeeMasterAddUpdateModal from './EmployeeMasterAddUpdateModal'
...
const EmployeeMasterList = () => {
    ...
    const GetDistrictListData = async (pageNumber, searchKeywordValue) => {
        setLoader(true);
        try {
            const response = await GetEmployeeMasterList({
                pageSize: pageSize,
                pageNo: pageNumber - 1,
    ...
    return (
        <>
            <div className="card">
                <div className="card-body p-2 bg-white shadow-md rounded-lg" style={{ borderRadius: '10px' }}>
                    <div className="d-flex justify-content-between align-items-center mb-1">
                        <h5 className="m-0">Employee Master</h5>
                                        ...
                                        <tr className='text-nowrap text-center' key={item.idx}>
                                            ...
                                            <td className="text-center py-2">
                                                {item.employeeName?.length > 30 ? (
                                                    <Tooltip title={item.employeeName}>{`${item.employeeName?.substring(0, 30)}...`}</Tooltip>
                                                ) : (
                                                    <>{item.employeeName}</>
                                                )}
                                            </td>
            ...
            {showAddUpdateModal &&
                <EmployeeMasterAddUpdateModal show={showAddUpdateModal} onHide={(() => setShowAddUpdateModal(false))} modelRequestData={modelRequestData} setIsAddUpdateDone={setIsAddUpdateDone} />
            }
            <ImagePreviewModal
                show={showModal}
