...
import TemplePujaCategoryAddUpdateModal from './TemplePujaCategoryAddUpdateModal'
...
const TemplePujaCategoryList = () => {
      ...
      useEffect(() => {
            GetTemplePujaCategoryListData(1, null)
      }, [])

      useEffect(() => {
            if (isAddUpdateDone) {
                  GetTemplePujaCategoryListData(currentPage, null)
                  setIsAddUpdateDone(false)
            }
      }, [isAddUpdateDone])

      // ------------API Callings--------------------
      const GetTemplePujaCategoryListData = async (pageNumber, searchKeywordValue) => {
            setLoader(true);
            try {
                  const response = await GetTemplePujaCategoryList({
                        pageSize: pageSize,
                        pageNo: pageNumber - 1,
      ...
      const updateBtnClicked = (value) => {
            setModelRequestData((prev) => ({
                  ...prev, Action: 'Update',
                  tempPujaCatID: value?.tempPujaCatID
            }))
            setShowAddTempleModal(true)
      }

      const fetchSearchResults = (searchValue) => {
            GetTemplePujaCategoryListData(currentPage, searchValue);
      };
      ...
      const confirmStatusChange = async (item) => {
            try {
                  const { tempPujaCatID } = item; // Destructure to access only what's needed
                  const response = await ChangeTemplePujaCategoryStatus(tempPujaCatID);
      ...
      return (
            <>
                  <div className="card">
                        <div className="card-body p-2 bg-white shadow-md rounded-lg" style={{ borderRadius: '10px' }}>
                              <div className="d-flex justify-content-between align-items-center mb-1">
                                    <h5 className="m-0">Temples Puja Category</h5>
                                                            ...
                                                            <tr className='text-nowrap text-center' key={item.idx}>
                                                                  ...
                                                                  <td className="text-center actionColSticky" style={{ zIndex: 4 }}>
                                                                        <div className="d-flex gap-2">
                                                                              <Tooltip title="Update Temple Puja Category Name">
                  ...
                  {showAddTempleModal &&
                        <TemplePujaCategoryAddUpdateModal show={showAddTempleModal} onHide={() => setShowAddTempleModal(false)} modelRequestData={modelRequestData} setIsAddUpdateDone={setIsAddUpdateDone} />
                  }
...
export default TemplePujaCategoryList
