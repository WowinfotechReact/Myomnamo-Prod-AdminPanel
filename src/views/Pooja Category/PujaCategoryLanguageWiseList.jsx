...
const PujaCategoryLanguageWiseList = () => {
    ...
    useEffect(() => {
        GetPujaCategoryListData(1, null)
    }, [])
    ...
    const GetPujaCategoryListData = async (pageNumber, searchKeywordValue) => {
        setLoader(true);
        try {
            ...
            }, location?.state?.data?.pujaCatKeyID);
    ...
    const confirmStatusChange = async (item, user) => {
        ...
        try {
            const { pujaCatKeyID, appLangID } = item; // Destructure to access only what's needed
            const response = await ChangePujaCategoryStatus(pujaCatKeyID, appLangID);
    ...
    const fetchSearchResults = (searchValue) => {
        GetPujaCategoryListData(1, searchValue);
    };
    ...
    const handlePageChange = (pageNumber) => {
        ...
        GetPujaCategoryListData(pageNumber, null);
    };
    ...
    const AddBtnClicked = () => {
        setModelRequestData((prev) => ({ ...prev, Action: null, appLangID: null, pujaCatKeyID: location?.state?.data?.pujaCatKeyID, pujaCatByLangKeyID: null }))
        setShowAddUpdateModal(true)
    }
    const updateBtnClickPujaCat = (value) => {
        setModelRequestData((prev) => ({
            ...prev, Action: 'Update',
            pujaCatKeyID: location?.state?.data?.pujaCatKeyID,
            pujaCatByLangKeyID: value?.pujaCatByLangKeyID,
...
export default PujaCategoryLanguageWiseList
